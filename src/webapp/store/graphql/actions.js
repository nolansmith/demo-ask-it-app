...
export const updateQuestions = (questions) => (dispatch, getState, { client }) => {
...
export const getAllQuestions = () => (dispatch, getState, { client }) => {
    ...
    client.query({ query: queries.GET_ALL_QUESTIONS }).then(({ error, data, loading }) => {
...
export const getLatestQuestions = () => (dispatch, getState, { client }) => {
    ...
    client.query({ query: queries.GET_LATEST_QUESTIONS }).then(({ error, data, loading }) => {
        //console.log('ReduxGQL: ', data);
        if (!loading) {
            dispatch({ type: "GET_LATEST_QUESTIONS", questions: data.getLatestQuestions });
            dispatch(updateLoading(false));
...
export const getSingleQuestion = (id) => (dispatch, getState, { client }) => {
    ...
    client.query({ query: queries.GET_SINGLE_QUESTION(id) }).then(({ error, data, loading }) => {
...
export const addVote = (vote) => (dispatch,getState,{client}) => {
    ...
    client.mutate({
        mutation: mutations.ADD_VOTE, variables: {
            vote: vote
        }
...
export const addNewUser = (user) => (dispatch, getState, { client }) => {
    ...
    client.query({ query: queries.FIND_USER(user.username) }).then(({ loading, data, error }) => {
        console.log(data);
...
export const askQuestion = (question) => (dispatch, getState, { client }) => {
    ...
    client.mutate({
        mutation: mutations.ASK_QUESTION, variables: {
            question: question
        }
    })
        .then(({ error, data, loading }) => {
            if (!loading) {
                ...
                dispatch(
                    { type: "UPDATE_QUESTIONS", questions: {asked: {id: data.askQuestion.id }}
                });
                dispatch(updateLoading());
